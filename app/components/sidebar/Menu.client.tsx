  ...
  const deleteItem = useCallback((event: React.UIEvent, item: ChatHistoryItem) => {
    ...
    if (db) {
      deleteById(db, item.id)
        ...
        .catch((error) => {
          toast.error('Falha ao deletar a conversa');
          logger.error(error);
        });
    }
  }, []);
    ...
    <>
      ...
      >
        <div className="h-12 flex items-center justify-between px-4 border-b border-gray-100 dark:border-gray-800/50 bg-gray-50/50 dark:bg-gray-900/50">
          ...
          <div className="flex items-center gap-3">
            <span className="font-medium text-sm text-gray-900 dark:text-white truncate">
              {profile?.username || 'Usuario Convidado'}
            </span>
        ...
        <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
          ...
          <div className="text-gray-600 dark:text-gray-400 text-sm font-medium px-4 py-2">Suas Conversas</div>
          <div className="flex-1 overflow-auto px-3 pb-3">
            {filteredList.length === 0 && (
              <div className="px-4 text-gray-500 dark:text-gray-400 text-sm">
                {list.length === 0 ? 'Nenhuma conversa anterior' : 'Nenhuma correspondência encontrada'}
              </div>
            )}
                  ...
                  <>
                    <div className="p-6 bg-white dark:bg-gray-950">
                      ...
                      <DialogDescription className="mt-2 text-gray-600 dark:text-gray-400">
                        ...
                        <p className="mt-2">Você tem certeza de que deseja excluir este chat?</p>
                      </DialogDescription>
                    </div>
